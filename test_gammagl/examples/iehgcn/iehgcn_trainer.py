...
def main(args):
    ...
    if str.lower(args.dataset) == 'imdb':
        ...
        # tensorflow后端下对于grad为None没有完善的解决方式，所以下列只支持torch和paddle后端下运行
        ...
        # test_idx = mask_to_index(graph[targetType[str.lower(args.dataset)]].test_mask)
        # val_idx = mask_to_index(graph[targetType[str.lower(args.dataset)]].val_mask)
        # num_nodes_dict = {'movie': graph['movie'].num_nodes, 'actor': graph['actor'].num_nodes,
        #                   'director': graph['director'].num_nodes}
...
if __name__ == '__main__':
    ...
    # import numpy as np
    # for i in range(5):
    #     test_macrof1.append(main(args))
    # test_macrof1_mean = np.mean(test_macrof1)
    # test_macrof1_std = np.std(test_macrof1)
    # print("test_macrof1: ")
    # print(test_macrof1)
    # print("test_macrof1_mean: ", test_macrof1_mean, "test_macrof1_std: ", test_macrof1_std)
